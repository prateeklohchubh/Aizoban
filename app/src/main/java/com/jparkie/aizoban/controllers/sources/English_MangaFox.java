...
/**
 * Created by prateek on 12/18/2015.
 */
public class English_MangaFox implements Source {
    ...
    public static final String BASE_URL = "www.mangafox.me";
...
//    }
    ...
    private String findNextUrlFromUpdatePageMarker(UpdatePageMarker newUpdate) {
        ...
        if (!requestUrl.equals(DefaultFactory.UpdatePageMarker.DEFAULT_NEXT_PAGE_URL)) {
            Log.d(this.getClass().getName(), "Reached here");
            Log.d(this.getClass().getName(), "Parsed Page number is: " + (requestUrl.substring(0, requestUrl.length()).replaceAll("[^0-9]+", "")));//working till here
            //Have to check for Null string returned
            String currentPageNumber = requestUrl.substring(0, requestUrl.lastIndexOf("htm")).replaceAll("[^0-9]+", "");
